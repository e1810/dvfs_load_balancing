...
mpirun -n 4 ./ompt_dvfs_example | tee data_ompt.out
...
printf "\nExecution time: %f seconds\n" "$(awk -v s="$start_time" -v e="$end_time" 'BEGIN{printf "%.6f", e-s}')" | tee -a data_ompt.out
printf "Whole-program energy: %s J\n" "$energy_j" | tee -a data_ompt.out
printf "Whole-program average power: %s W\n" "$avg_w" | tee -a data_ompt.out
